                ...
                <div className="overflow-y-auto no-scrollbar px-5 py-5 space-y-4 custom-scrollbar">
                  ...
                  <div>
                    ...
                    <div className="bg-white p-2 rounded-2xl space-y-1 border border-toss-grey-100 shadow-sm">
                      ...
                      <div className="flex items-center justify-between px-4 py-3.5 border-t border-toss-grey-100">
                        <div><span className="text-[14px] font-black text-toss-grey-800">광고 클릭 보너스 혜택</span><p className="text-[11px] text-toss-grey-400 mt-0.5">클릭 시 1시간 대기 없이 바로 주문 확인</p></div>
                        <button onClick={() => handleUpdateSettings({ ...appSettings, adFreeRewardEnabled: !appSettings.adFreeRewardEnabled })} className={`w-11 h-6 rounded-full transition-all relative shadow-inner shrink-0 ml-3 ${appSettings.adFreeRewardEnabled ? 'bg-toss-green' : 'bg-toss-grey-300'}`}><div className={`absolute top-[2px] left-[2px] w-5 h-5 bg-white rounded-full transition-all duration-300 transform shadow-sm ${appSettings.adFreeRewardEnabled ? 'translate-x-[20px]' : 'translate-x-0'}`} /></button>
                      </div>
                    </div>
                  </div>
